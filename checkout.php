...
if(isset($_POST['submit'])){
   ...
   if($check_cart->rowCount() > 0){
      ...
      if($address == ''){
         ...
         $insert_order = $conn->prepare("INSERT INTO `orders`(user_id, name, number, email, address, total_products, total_price, payment_status,placed_on) VALUES(?,?,?,?,?,?,?,?,?)");
         $insert_order->execute([$user_id, $name, $number, $email, $address, $total_products, $total_price, $payment_status, $placed_on]);

         if($payment_status == 'Credit Card'){
            header('location:payment.php');
         }
         else if($payment_status == 'Cash on Delivery'){
            ...
            $message[] = 'order placed successfully!';
               ...
               $message[] = 'select credit card or cash on delivery!!';
            }
...
<form action="" method="post">
   ...
   <div class="cart-items">
      ...
      <a href="cart.php" class="click_btn">veiw cart</a>
   </div></br>
   ...
   <div class="user-info">
      ...
      <a href="update_address.php" class="click_btn">update address</a>

   </br></br></br></br>

   <div class="box">
      <input type="hidden" name="payment_status" value="<?= $fetch_orders['payment_status']; ?>">
      <input type="radio" name="payment_status" value="Cash on Delivery"> Cash on Delivery
      &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp
      <input type="radio" name="payment_status" value="Credit Card"> Credit Card 
   </div>
   <input type="hidden" name="placed_on" value="<?= $fetch_orders[date('Y-m-d')]; ?>">
